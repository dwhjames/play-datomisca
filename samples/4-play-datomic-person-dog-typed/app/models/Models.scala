...
object Dog {
  ...
  def getIdByName(dogName: String)(implicit database: Database): Option[Long] = {
    Datomic.q(queryDogByName, database, dogName).headOption map {
      case dogId: Long => dogId
    }
  }  
...
object Person {
  ...
  def jsonReads(implicit database: Database): Reads[PartialAddEntity] = (
    ...
    (__ \ 'characters).read( readAttr[Set[Keyword]](Person.characters) )
    reduce
  )
  ...
  def jsonWrites = (
    ...
    (__ \ "dog")       .write(writeAttr[Keyword](Person.dog))  and
    (__ \ "characters").write(writeAttr[Set[Keyword]](Person.characters))
    join
  )
}